...
<h3>
    <samp>用于并行编程的语言和编译器<br>
    通过基于 Python 的 DSL 简化 GPU 内核开发。</samp>
</h3>
...
- **基于 Python 的 DSL**: 使用熟悉的 Python 语法编写 GPU 内核
- **基于瓦片的编程模型**: 通过瓦片计算实现高效并行执行
- **即时编译**: 使用 `@tileon.jit` 装饰器进行动态内核编译
...
### 神经网络原语
- **Softmax**: 行级 softmax 计算
- **Flash Attention**: 高效注意力机制
- **Block-Sparse Attention**: 具有自定义块模式的稀疏注意力
...
- `tl_random.rand`: 均匀随机 [0, 1)
- `tl_random.randn`: 正态分布 N(0, 1)
- `tl_random.randint`: 整数随机值

## 📄 许可证

Tileon 基于 MIT 许可证授权。详见 [LICENSE](LICENSE.md)。
