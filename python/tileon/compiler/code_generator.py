...
def _is_constexpr(o: Any) -> bool:
    return o is None or isinstance(
        o, (constexpr, language.core.dtype, JITCallable))
...
class ASTFunction:
    ...
    def flatten_ir_types(self, builder: ir.builder,
                         types: List[base_type]) -> List[ir.type]:
        ir_types = []
        for ty in types:
            if ty is None:
                continue
...
class CodeGenerator(ast.NodeVisitor):
    ...
    def __init__(self,
        ...
        if is_kernel:
            ...
            function_name = check_identifier_legality(function_name,
                                                      "function")
    ...
    def _define_name_lookup(self):
        ...
        def global_lookup(name: str, absent):
            ...
            if any([
                    ...
                    getattr(val, "__module__",
                            "").startswith("triton.language"),  #
    ...
    def set_value(self, name: str, value: Union[base_value,
                                                constexpr]) -> None:
        ''' This function:
    ...
    def visit_ListComp(self, node: ast.ListComp):
        ...
        if not isinstance(iter, tl_tuple):
            raise NotImplementedError(
                "only tuple comprehensions are supported")
        ...
        def common_type(a, b):
            if isinstance(a, tl.tuple_type):
                ...
                return tl.tuple_type(
                    [common_type(ai, bi) for ai, bi in zip(a, b)],
                    fields=a.fields)
    ...
    def visit_FunctionDef(self, node):
        ...
        for i, default_value in enumerate(node.args.defaults[::-1]):
            ...
            if annotation is None:
                init_node = ast.Assign(targets=[st_target],
                                       value=default_value)
            else:
                init_node = ast.AnnAssign(target=st_target,
                                          value=default_value,
        ...
        self.fn = self.builder.get_or_insert_function(self.module,
                                                      self.function_name,
                                                      fn_ty, visibility,
                                                      self.noinline)
    ...
    def visit_arg(self, node):
        ...
        if param.is_constexpr and (param.do_not_specialize
                                   or param.do_not_specialize_on_alignment):
    ...
    def assignTarget(self, target, value):
        ...
        if isinstance(target, ast.Attribute):
            raise NotImplementedError(
                "Attribute assignment is not supported in triton")
    ...
    def visit_Assign(self, node):
        ...
        targets = [node.target] if isinstance(node,
                                              ast.AnnAssign) else node.targets
    ...
    def visit_BinOp(self, node):
        ...
        if method_name is None:
            raise self._unsupported(
                node,
                "AST binary operator '{}' is not (currently) implemented.".
                format(node.op.__name__))
    ...
    def visit_if_top_level(self, cond, node):
        with enter_sub_region(self) as sr:
            ...
            self.builder.create_cond_branch(cond.handle, then_block,
                                            else_block)
    ...
    def visit_If(self, node):
        ...
        if _is_triton_tensor(cond):
            ...
            if cond.type.is_block():
                ...
                cond = language.core._unsplat(cond,
                                              _semantic=self.semantic,
                                              _generator=self)
            ...
            if type(cond) not in _condition_types:
                raise self._unsupported(
                    node,
                    "`if` conditionals can only accept values of type {{{}}}, not objects of type {}"
                    .format(', '.join(_.__name__ for _ in _condition_types),
                            type(cond).__name__))
        ...
        if _is_triton_tensor(cond):
            ...
            with enter_sub_region(self):
                ...
                ret_type_ir = [ret_type.to_ir(self.builder)
                               ] if ret_type != language.void else []
                if_op = self.builder.create_if_op(ret_type_ir, cond.handle,
                                                  True)
                then_block.merge_block_before(if_op.get_then_block())
                if ret_type_ir:
                ...
                return language.core.tensor(if_op.get_result(0),
                                            ret_type) if ret_type_ir else None
        else:
            cond = _unwrap_if_constexpr(cond)
    ...
    def visit_Compare(self, node):
        ...
        if method_name is None:
            raise self._unsupported(
                node,
                "AST comparison operator '{}' is not (currently) implemented.".
                format(node.ops[0].__name__))
        return self._apply_binary_method(node, method_name, lhs, rhs)
    ...
    def _verify_loop_carried_variable(self, name, loop_val, live_val):
        assert _is_triton_value(
            loop_val), f'cannot reassign constexpr {name} in the loop'
        assert _is_triton_value(
            live_val), f'cannot reassign constexpr {name} in the loop'
    ...
    def visit_While(self, node):
        with enter_sub_region(self) as sr:
            ...
            names, init_handles, init_fe_tys = self._find_carries(
                node, liveins)
            ...
            block_args = [
                before_block.arg(i) for i in range(len(init_handles))
            ]
            ...
            body_handles = [
                after_block.arg(i) for i in range(len(init_handles))
            ]
            ...
            yield_handles = flatten_values_to_ir(self.lscope[name]
                                                 for name in names)
            self.builder.create_yield_op(yield_handles)

        # WhileOp defines new values, update the symbol table (lscope, local_defs)
        result_handles = [
            while_op.get_result(i) for i in range(len(init_handles))
        ]
    ...
    def visit_For(self, node):
        ...
        iter_kwargs = dict(
            self.visit(keyword) for keyword in node.iter.keywords)
        if IteratorClass == language.static_range:
            iterator = IteratorClass(*iter_args, **iter_kwargs)
            static_range = range(iterator.start.value, iterator.end.value,
                                 iterator.step.value)
        ...
        if IteratorClass is language.range:
            ...
            # note: only `range` iterator is supported now
            # collect lower bound (lb), upper bound (ub), and step
            lb = iter_args[0] if len(iter_args) > 1 else self.visit(
                ast.Constant(0))
            ub = iter_args[1] if len(iter_args) > 1 else self.visit(
                node.iter.args[0])
            step = iter_args[2] if len(iter_args) > 2 else self.visit(
                ast.Constant(1))
        else:
            raise RuntimeError(
        ...
        if not lb.dtype.is_int() or not ub.dtype.is_int(
        ) or not step.dtype.is_int():
            raise TypeError(
        ...
        self.set_value(node.target.id,
                       language.core.tensor(iv_placeholder, iv_type))
        ...
        with enter_sub_region(self) as sr:
            ...
            if disallow_acc_multi_buffer:
                for_op.set_attr("tt.disallow_acc_multi_buffer",
                                self.builder.get_unit_attr())
            if flatten:
                for_op.set_attr("tt.flatten", self.builder.get_unit_attr())
            if warp_specialize:
                for_op.set_attr("tt.warp_specialize",
                                self.builder.get_unit_attr())
            if disable_licm:
                for_op.set_attr("llvm.loop_annotation",
                                self.builder.get_disable_loop_licm_attr())
            ...
            block_handles = [
                for_op_body.arg(i + 1) for i in range(len(init_handles))
            ]
            ...
            assert for_op_region.size(
            ) == 1, "We use SCF, so the loop body should only have one block"
        ...
        result_handles = [
            for_op.get_result(i) for i in range(len(init_handles))
        ]
    ...
    def call_JitFunction(self,
        ...
        handles = [
            call_op.get_result(i) for i in range(call_op.get_num_results())
        ]
    ...
    def call_Function(self, node, fn, args, kws):
        ...
        if (hasattr(fn, '__self__') and _is_triton_value(
                fn.__self__)) or language.core.is_builtin(fn) or isinstance(
        ...
        if fn in self.builtin_namespace.values() or (hasattr(
                fn, '__self__') and not _is_triton_value(fn.__self__)):
    ...
    def visit_Call(self, node):
        ...
        if not isinstance(fn, BoundJITMethod):
            static_implementation = self.statically_implemented_functions.get(
                fn)
    ...
    def visit_BoolOp(self, node: ast.BoolOp):
        ...
        if method_name is None:
            raise self._unsupported(
                node,
                "AST boolean operator '{}' is not (currently) implemented.".
                format(node.op.__name__))
        ...
        for i, value in enumerate(values):
            if isinstance(value, ast.Constant):
                ...
                values[i] = ("{}" if conversion_code < 0 else "{!" +
                             chr(conversion_code) + "}").format(
                                 evaluated.value)
            else:
                raise AssertionError(
                    "encountered unexpected node of type {} in a JoinedStr node"
                    .format(type(value)))
        return ''.join(values)
    ...
    def visit(self, node):
        ...
        if hasattr(node, 'lineno') and hasattr(node, 'col_offset'):
            here_loc = self.builder.create_loc(self.file_name,
                                               self.begin_line + node.lineno,
                                               node.col_offset)
            if self.name_loc_as_prefix is not None:
                self.builder.set_loc(
                    self.builder.create_name_loc(self.name_loc_as_prefix,
                                                 here_loc))
            else:
        ...
        if not passed:
            if arg_count == 1:
                ...
                try:
                    ...
                    message = "<failed to evaluate assertion message: " + repr(
                        e) + ">"

            raise CompileTimeAssertionFailure(self.jit_fn.src, node,
                                              _unwrap_if_constexpr(message))
        return None
    ...
    def static_executor(python_fn):
        ...
        def ret(self, node: ast.Call):
            kws = {
                ...
                for name, value in (self.visit(keyword)
    ...
    statically_implemented_functions: Dict[object,
                                           Callable[[ast.Call], Any]] = {
                                               language.core.static_assert:
                                               execute_static_assert,
                                               language.core.static_print:
                                               static_executor(print),
                                               ttgl.static_assert:
                                               execute_static_assert,
                                               ttgl.static_print:
                                               static_executor(print),
