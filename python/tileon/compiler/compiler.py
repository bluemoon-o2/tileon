...
class ASTSource:
    ...
    def make_ir(self, target: GPUTarget, options, codegen_fns, module_map,
                context):
        from .code_generator import ast_to_ttir
        return ast_to_ttir(self.fn,
                           self,
                           context=context,
                           options=options,
...
class IRSource:
    ...
    def __init__(self, path, context, backend):
        ...
        if self.ext == "ptx":
            match = re.search(prototype_pattern[self.ext], self.src,
                              re.MULTILINE)
            self.name = match.group(1)
            signature = match.group(2)
            types = re.findall(arg_type_pattern[self.ext], signature)
            self.signature = {
                k: convert_type_repr(ty)
...
def filter_traceback(e: BaseException):
    ...
    while tb is not None:
        if not any(f for f in BAD_FILES
                   if tb.tb_frame.f_code.co_filename.endswith(f)):
            frames.append(tb)
        tb = tb.tb_next
...
class CompileTimer:
    ...
    def end(self) -> knobs.CompileTimes:
        ...
        for stage_name, stage_end in self.lowering_stage_ends:
            lowering_stage_durations.append(
                (stage_name, delta(stage_start, stage_end)))
...
def compile(src, target=None, options=None, _env_vars=None):
    ...
    ) if _env_vars is None else _env_vars
    ...
    for ext, compile_ir in list(stages.items())[first_stage:]:
        ...
        if fn_override_manager is None:
            ...
            if (ir_override := metadata.get(
                    "ir_override", None)) and ir_override.endswith(f".{ext}"):
...
def make_backend(target: GPUTarget) -> BaseBackend:
    actives = [
        x.compiler for x in backends.values()
...
class CompiledKernel:
    ...
    def __init__(self, src, metadata_group, hash):
        ...
        metadata_path = next((Path(p) for c, p in metadata_group.items()
                              if c.endswith(".json")))
        metadata = json.loads(metadata_path.read_text())
        # JSON serialization dumps the target as a dict. Restore it to a GPUTarget.
        target = metadata['target']
        metadata['target'] = GPUTarget(target['backend'], target['arch'],
                                       target['warp_size'])
        KernelMetadata = namedtuple('KernelMetadata',
        ...
        asm_files = [
            Path(p) for c, p in metadata_group.items()
    ...
    def __del__(self):
        ...
        if self.module is not None:
            if knobs.runtime.module_unload_hook is not None:
                knobs.runtime.module_unload_hook(self.module, self.function,
                                                 self.name,
                                                 self.metadata_group,
                                                 self.hash)
    ...
    def _init_handles(self):
        ...
        if self.metadata.shared > max_shared:
            raise_(
                OutOfResources(self.metadata.shared, max_shared,
                               "shared memory"))
        if hasattr(self.metadata,
                   "tmem_size") and self.metadata.tmem_size is not None:
            # Use blackwell max tmem size for now, this should be moved in device properties
            max_tmem_size = 512  # tmem size in number of columns
            if self.metadata.tmem_size > max_tmem_size:
                raise_(
                    OutOfResources(self.metadata.tmem_size, max_tmem_size,
                                   "tensor memory"))
        if knobs.runtime.kernel_load_start_hook is not None:
            knobs.runtime.kernel_load_start_hook(self.module, self.function,
                                                 self.name,
                                                 self.metadata_group,
                                                 self.hash)
        ...
        if self.metadata.num_warps * warp_size > self.n_max_threads:
            raise_(
                OutOfResources(self.metadata.num_warps * warp_size,
                               self.n_max_threads, "threads"))
        if knobs.runtime.kernel_load_end_hook is not None:
            knobs.runtime.kernel_load_end_hook(self.module, self.function,
                                               self.name, self.metadata_group,
                                               self.hash)
    ...
    def launch_metadata(self, grid, stream, *args):
        ...
        if not isinstance(self.src,
                          ASTSource) or self.src.fn.launch_metadata is None:
            return ret
        arg_dict = {
            name: arg
            for name, arg in zip(self.src.fn.arg_names, args)
        }
    ...
    def __getitem__(self, grid):
        ...
        def runner(*args, stream=None):
            ...
            self.run(grid[0], grid[1], grid[2], stream, self.function,
                     self.packed_metadata, launch_metadata,
