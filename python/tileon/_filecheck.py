...
import tileon
from tileon.compiler import ASTSource, make_backend
from tileon.backends.compiler import GPUTarget
from tileon.runtime.jit import create_function_from_signature
from tileon._C.libtriton import ir
...
tileon_dir = os.path.dirname(__file__)
filecheck_path = os.path.join(tileon_dir, "FileCheck")
...
def run_filecheck(name: str, module_str: str, check_template: str) -> None:
    with tempfile.TemporaryDirectory() as tempdir:
        ...
        # Write the expected output to a temporary file.
        ...
        try:
            subprocess.check_output([
                filecheck_path, temp_expected, "--input-file", temp_module,
...
def run_parser(kernel_fn, args=(), kwargs={}, target=stub_target):
    # Overflow checks are disabled by default for testing
...
def run_filecheck_test(kernel_fn):
    assert isinstance(kernel_fn, tileon.runtime.JITFunction)
