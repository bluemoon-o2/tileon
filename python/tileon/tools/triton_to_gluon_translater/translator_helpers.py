...
def tl_dot_mma_sync_k_width(a_ty, b_ty):
    a_bitwidth = a_ty.element_t.primitive_bitwidth
    b_bitwidth = b_ty.element_t.primitive_bitwidth
...
def tl_dot_mmav5_supported(a_ty, b_ty, num_warps, input_precision, allow_tf32, max_num_imprecise_acc):
    ...
    N = b_ty.shape[1]
    K = a_ty.shape[1]
    min_K = 256 // a_ty.element_t.primitive_bitwidth
    if a_ty.element_t.is_int() or b_ty.element_t.is_int():
        return False
    if min(a_ty.element_t.primitive_bitwidth, b_ty.element_t.primitive_bitwidth) >= 32 and input_precision != "tf32":
        return False
...
def get_shared_memory_mma_layout(type, operand_index, allow_transpose, is_fp4_padded=False, force_transpose=False):
    ...
    ele_bit_width = type.element_t.primitive_bitwidth
...
def tl_dot_blackwell(a, b, acc=None, input_precision=None, allow_tf32=None, max_num_imprecise_acc=None,
    ...
    allow_transpose = not a.type.element_t.is_fp32()
...
def tl_make_tensor_descriptor(base, shape, strides, block_shape, padding_option="zero", _semantic=None):
    layout = ttgl.NVMMASharedLayout.get_default_for(block_shape, base.dtype.element_t)
