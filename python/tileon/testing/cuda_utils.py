    ...
    def decorator(test_fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if run_cuda_memcheck and ppid_name != "cuda-memcheck":
                ...
                out = subprocess.run(["cuda-memcheck", "pytest", "-vs", cmd],
                                     capture_output=True,
                                     env=env)
...
def set_gpu_clock(ref_sm_clock: int = 1350, ref_mem_clock: int = 1215):
    ...
    try:
        ...
        assert abs(
            cur_sm_clock -
            ref_sm_clock) < 10, f"GPU SMs must run at {ref_sm_clock} MHz"
        assert abs(
            cur_mem_clock -
            ref_mem_clock) < 10, f"GPU SMs must run at {ref_mem_clock} MHz"
