...
from ..language import core as tl
from .._C import ir
...
class TileonSemantic(Generic[Tensor_t]):
    """The semantic of the Tileon language."""

    tensor: Type[Tensor_t] = tl.tensor
    lang = tl
    ...
    def to_tensor_type(self, x):
        if isinstance(x, tl.dtype):
            return x
        elif isinstance(x, tl.constexpr_t):
            x = x.value
...
# ===----------------------------------------------------------------------===//
    ...
    def _bool_like(self, v: Tensor_t) -> tl.block_t:
        return v.type.with_element_t(tl.int1)

    def greater_than(self, input: Tensor_t, other: Tensor_t) -> Tensor_t:
        input, other = self.binary_op_type_checking_impl(input, other)
        scalar_ty = input.type.scalar
        # float > float
        if scalar_ty.is_floating():
...
# ===----------------------------------------------------------------------===//
    ...
    def full(self, shape: List[int], value, dtype: tl.dtype) -> Tensor_t:
        return self.splat(self.make_scalar(value, dtype), shape)

    def splat(self, value: Tensor_t, shape: List[int]) -> Tensor_t:
    ...
    def join(self, a: Tensor_t, b: Tensor_t) -> Tensor_t:
        ...
        ret_type = tl.block_t(a.type.scalar, new_shape)
    ...
    def split(self, a: Tensor_t) -> Tuple[Tensor_t, Tensor_t]:
    ...
    def broadcast_impl_value(self, lhs: Tensor_t, rhs: Tensor_t) -> Tensor_t:
        lhs_ty = lhs.type
        rhs_ty = rhs.type

        # make_shape_compatible(block, scalar)
        if lhs_ty.is_block() and not rhs_ty.is_block():
            rhs_ty = lhs_ty.with_element_t(rhs_ty.scalar)
        ...
        # make_shape_compatible(scalar, block)
            ...
            lhs_ty = rhs_ty.with_element_t(lhs_ty.scalar)
            lhs = self.tensor(self.builder.create_splat(lhs_ty.to_ir(self.builder), lhs.handle), lhs_ty)
        # make_shape_compatible(block, block)
        elif lhs_ty.is_block() and rhs_ty.is_block():
            lhs_shape = lhs_ty.get_block_shape()
            rhs_shape = rhs_ty.get_block_shape()
...
#######
    ...
    def bitcast(self, input: Tensor_t, dst_ty: tl.dtype) -> Tensor_t:
        src_ty = input.type
        if src_ty.is_block():
            dst_ty = src_ty.with_element_t(dst_ty.scalar)
        if src_ty == dst_ty:
            return input
    ...
    def cast(self,
        ...
        if src_sca_ty == dst_sca_ty:
            return input
        if src_ty.is_block():
            dst_ty = src_ty.with_element_t(dst_sca_ty)
...
# ===----------------------------------------------------------------------===//
    ...
    def _load_legacy(self, ptr, mask, other, boundary_check, padding, cache,
        ...
        if is_bool:
            elt_ty = tl.int8
            ptr_ty = tl.pointer_t(elt_ty, ptr_ty.address_space)
        ...
        if ptr.type.is_block():
            dst_ty = ptr.type.with_element_t(elt_ty)
        else:
    ...
    def descriptor_store(self, desc: tl._tensor_descriptor, value: Tensor_t, offsets) -> Tensor_t:
