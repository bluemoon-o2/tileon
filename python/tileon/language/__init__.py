...
def str_to_t(name, c):
    ...
    if isinstance(name, tuple):
        ...
        return tuple_t([str_to_t(x, c) for x in name], fields)
    ...
    if name[0] == "*":
        ...
        if name[0] == "k":
            ...
            const = True
        t = str_to_t(name, c)
        return pointer_t(element_t=t, const=const)
    ...
    if name.startswith("tensordesc"):
        ...
        shape_type = tuple_t([int32] * ndim)
        # FIXME: Last dim stride should be constexpr(1)
        stride_type = tuple_t(([int64] * ndim))
        block = block_t(dtype, block_shape)
        return tensor_descriptor_t(block, shape_type, stride_type)

    if name.startswith("constexpr"):
        return constexpr_t(c)
