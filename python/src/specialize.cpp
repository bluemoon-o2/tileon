...
using DtypePtr2Str = std::unordered_map<DTypePtrKey, py::object, DTypePtrKeyHash>;
using Dtype2Str = std::unordered_map<DTypeKey, py::object>;

// Handler signature using py::handle for efficiency (no refcount churn)
using TypeHandler = std::pair<py::object, py::object> (*)(py::handle, py::handle, bool, bool, bool);
using TypeHandlerCache = std::unordered_map<PyTypeObject *, TypeHandler>;

// Forward declarations
static std::pair<py::object, py::object>
specialize_arg(py::handle backend, py::handle arg, bool is_const, bool specialize_value, bool align);
...
void init_interned_strings() {
  ...
  has_native_tensor_spec_attr = intern_from_string("supports_native_tensor_specialization");
...
std::pair<py::object, py::object> specialize_tensordesc(py::handle arg,
  ...
  if (it != dtype2str.end()) {
    ...
    py::object res = py::reinterpret_steal<py::object>(
        PyObject_CallFunctionObjArgs(canonicalize_dtype_fn, dtype.ptr(), nullptr));
    if (!res) return {};
    dtype2str[dsk] = res;
    type_str_obj = res;
  }
  ...
  if (!block_shape_obj) return {};
  
  // Convert block_shape to list then string
  py::object block_shape_list = py::reinterpret_steal<py::object>(PySequence_List(block_shape_obj.ptr()));
  if (!block_shape_list) return {};
...
std::pair<py::object, py::object> handle_long_type(py::handle backend, py::handle arg, bool is_const,
  ...
  if (overflow == 0) {
    ...
    if (PyErr_Occurred()) {
      PyErr_SetString(PyExc_OverflowError, "integer to be specialized too large to represent");
      return {};
    }
...
std::pair<py::object, py::object> handle_tensor(py::handle backend, py::handle arg, bool is_const,
    bool specialize_value,  bool align) {
  ...
  if (it != dtype_ptr2str.end()) {
    ...
    py::object canon_res = py::reinterpret_steal<py::object>(
        PyObject_CallFunctionObjArgs(canonicalize_ptr_dtype_fn, dtype.ptr(), is_const ? Py_True : Py_False, nullptr)
    );
    if (!canon_res) return {};
  ...
  py::handle native_spec_obj = backend.attr(py::handle(has_native_tensor_spec_attr));
  if (native_spec_obj) {
  ...
  if (native_impl_available) {
    ...
    if (!data_ptr_result) return {};

    unsigned long long data_ptr = PyLong_AsUnsignedLongLong(data_ptr_result.ptr());
    if (PyErr_Occurred()) return {};
...
std::pair<py::object, py::object> handle_bool_type(py::handle backend, py::handle arg, bool is_const,
    bool specialize_value, bool align) {
...
std::pair<py::object, py::object>
handle_gluon_tensor_descriptor(py::handle backend, py::handle arg, bool is_const,
    bool specialize_value, bool align) {
...
std::pair<py::object, py::object> handle_tuple(py::handle backend, py::handle arg, bool is_const,
    bool specialize_value, bool align) {
...
bool init_globals() noexcept try {
  ...
  tensor_descriptor_cls = import_from("tileon.runtime.interpreter", "TensorDescriptor");
  canonicalize_dtype_fn = import_from("tileon._utils", "canonicalize_dtype");
  canonicalize_ptr_dtype_fn = import_from("tileon._utils", "canonicalize_ptr_dtype");
...
std::pair<py::object, py::object> specialize_arg(py::handle backend, py::handle arg, bool is_const,
    bool specialize_value, bool align) {
...
py::tuple specialize_impl(py::object backend, py::object arg, bool is_const, bool specialize_value, bool align) {
  if (!init_called) {
    if (!init_globals()) {
      throw py::error_already_set();
    }
  ...
  if (!type || !key) {
    if (!PyErr_Occurred()) {
      std::string type_name = py::type::of(arg).attr("__name__").cast<std::string>();
      throw py::type_error("failed to specialize argument of type: " + type_name);
    }
    throw py::error_already_set();
  }
...
void init_specialize(py::module_ &m) {
  ...
  m.def("native_specialize_impl", &specialize::specialize_impl, 
        "self"_a, "arg"_a, "is_const"_a, "specialize"_a, "align"_a,
        R"pbdoc(
